...
##### Ответ преподователя. 
Спасибо за проделаннную работу. Но есть недочет - поля и аргументы конструктора у NasaObject все равно надо называть в camelCase, для этого и нужна аннотация JsonProperty, чтобы значения json’ов в формате media_type знать в какое поле инжектировать(то есть провести соответствие media_type с аргументом конструктора/полем, в которое надо внедрить значение), соответственно, сами поля и аргументы конструктора вы можете писать в привычном camelCase. 
## Описание
